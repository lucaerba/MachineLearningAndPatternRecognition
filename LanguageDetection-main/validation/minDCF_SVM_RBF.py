...
PCA = [5, 4, 3, None] #number of dimension to keep in PCA
...
for K_num, K in enumerate(K_list):
    ...
    for PCA_m in PCA:
        ...
        plt.legend()
        fig.savefig(f"Results/minDCF_SVM_RBF_results/SVM_RBF_K{K_num}_PCA{PCA_m}.png", dpi=200)
        plt.show()
...
with open('Results/minDCF_SVM_RBF_results/SVM_RBF_ResultsTable.txt', 'w') as file:
    file.write(data)
